...
class Button extends Component {
  static propTypes = {
    upgrade: allValidators(PropTypes.bool, (props, propName) => {
      if (!props[propName]) {
        deprecationLog(
          'Using "Button" with current api is deprecated. In order to upgrade to the new Button api just use "<Button upgrade/>" and follow "5.1 Button" changed api docs. IMPORTANT! - After upgrading, when you import the buttonTestkitFactory, you will get an async testkit (all methods are async).',
        );
      }
    }),
  };
